...
[官方地址](https://github.com/wux-weapp/wx-extend)

功能：该库可以对微信小程序**form**提交的数据做校验。也可以自己将待校验数据封装成类似这样的格式进行校验：`{username: "admin", password: "123456", email: "123456@qq.com"}`。



## 具体代码

实现细节：表单内容放在`<form>`标签中；form标签的属性有`catchsubmit="formSubmit"`;每一个输入框都有一个`name`属性；提交按钮的属性为`formType="submit"`；接收form数据的JS函数是`formSubmit(e) {}`。其中，`formSubmit`函数名可以自定义。
...
​		a) `checkForm`函数传入的参数类似：`{username: "admin", password: "123456", email: "123456@qq.com"}`。

​		b) rules中的对象和message中的对象要一一对应，并且要和`checkForm`函数传入的参数严格对应，rules中设定校验规则，message中设定报错提示。

​		c) 校验规则可参考[官方文档](https://github.com/wux-weapp/wx-extend/blob/master/docs/components/validate.md#%E5%86%85%E7%BD%AE%E6%A0%A1%E9%AA%8C%E8%A7%84%E5%88%99)。
